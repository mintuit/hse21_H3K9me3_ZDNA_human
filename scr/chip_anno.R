...
peaks <- toGRanges(paste0(DATA_DIR, 'H3K9me3_SK-N-SH_intersect_DeepZ.bed'), format="BED")
peaks[1:2]
...
write.table(anno_df, file=paste0(DATA_DIR, 'H3K9me3_SK-N-SH_intersect_DeepZ.genes.txt'),
            col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)

anno_df %>% head()

uniq_genes_df <- unique(anno_df['symbol'])
write.table(uniq_genes_df, file=paste0(DATA_DIR, 'H3K9me3_SK-N-SH_intersect_DeepZ.genes_uniq.txt'),
